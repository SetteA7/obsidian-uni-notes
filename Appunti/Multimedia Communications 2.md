...
| **Partitioning**         | VBS: $16\times8$, $8\times16$, $8\times8$, $4\times4$ sub-MB | Quad-tree only (CU → PU + TU, independent trees)<br><br>PU how to code<br>TU how to transform | QT + MTT: binary (1:1) and ternary (1:2:1) splits; CU/PU/TU unified | 10-way split incl. 4:1 and 1:4 rectangular; recursive                 |
...
| **CRA** | Intra frame that does _not_ clear the DPB                                               | If seeking: decoder starts here, discards RASL frames (their references are missing) and decodes the rest cleanly. If playing normally: RASL frames are decoded using pre-CRA references → no compression loss | Introduced in HEVC as a cheaper seek point than IDR; lets the encoder maintain long-range temporal references across the access point                               |
| **BLA** | A CRA whose leading RASL references have been destroyed by a network node               | Decoder immediately drops all RASL frames without looking up references; rest decoded cleanly                                                                                                                  | Generated by transcoders/packagers/ad-splicers when they cut and stitch streams at a CRA; they flip the NALU header CRA → BLA to signal "leading frames are broken" |
...
$$J(n)=\lambda_1k_n-\lambda_2\abs{k_n-k_{n-1}}-\phi(\Delta_n)$$
And the full sequence QoE is
...
And the final rate should be lower than the throughput scaled by a safety factor $\eta$ usually 0.85:
$$R(n)\leq \eta\hat S(n)$$
This has 2 vulneabilities:
- $B(t)$ is ignored
- If $S>R_C$ then we are in the off region for a long time, this can cause TCP/QUIC to collapse the congestion window


**Buffer based ABR** is a solution:
- Cushion zone $0\leq B<B_{low}$: Lowest bitrate $R_\min$ to allow for smooth communication
- Active Mapping Zone $B_{low}\leq B\leq B_{high}$: we can improve resolution linearly
- Safety Reservoir $B>B_{high}$: Fixed $R_\max$ 
![[Pasted image 20260626144315.png|Example|250]]
This however:
- Knows about channel drops after the buffer empties
- Not optimized in low/high regions
- Cannot differentiate between channel or transport issues

**Hybrid ABR** keeps in mind also previous segments